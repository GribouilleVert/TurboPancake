...
class Validator {
    ...
    public function uploaded(string $field, ?string $customError = null): self
    {
        /**
    ...
    /**
     * Vérifie qu'un fichier uploadé correspond bien a une certaines liste de types mimes
    ...
    public function matchMimes(string $field, $mime, ?string $customError = null, bool $dotFileOnly = false): self
    {
        ...
        $currentMime = mime_content_type($filePath);
        ...
        if (is_array($mime)) {
            if (!in_array($currentMime, $mime)) {
                $this->addError($field, 'mimes', [implode(', ', $mime)], $customError);
            }
        } elseif (is_string($mime)) {
            if (!preg_match($mime, $currentMime)) {
                $this->addError($field, 'regexMime', [$mime], $customError);
            }
        } else {
